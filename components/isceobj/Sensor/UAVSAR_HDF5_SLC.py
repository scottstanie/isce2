...
try:
    ...
    raise ImportError("Python module h5py is required to process COSMO-SkyMed data")
...
class UAVSAR_HDF5_SLC(Sensor):
    ...
    def __init__(
        ...
        super(UAVSAR_HDF5_SLC, self).__init__(
    ...
    def _populatePlatform(self, file):
        ...
        platform.setMission(
            file["/science/LSAR/identification"].get("missionId")[()].decode("utf-8")
        )
        platform.setPointingDirection(
            self.lookMap[
                file["/science/LSAR/identification"]
                .get("lookDirection")[()]
                .decode("utf-8")
            ]
        )
    ...
    def _populateInstrument(self, file):
        ...
        rangePixelSize = file[
            "/science/LSAR/SLC/swaths/" + self.frequency + "/slantRangeSpacing"
        ][()]
        wvl = (
            SPEED_OF_LIGHT
            / file[
        ...
        Chirp_slope = 1.0
        rangeBandwidth = file[
            "/science/LSAR/SLC/swaths/" + self.frequency + "/processedRangeBandwidth"
        ][()]
        Chirp_length = rangeBandwidth / Chirp_slope
        instrument.setPulseLength(Chirp_length)
        instrument.setChirpSlope(Chirp_slope)
        rangeSamplingFrequency = SPEED_OF_LIGHT / 2.0 / rangePixelSize
    ...
    def _populateFrame(self, file):
        ...
        slantRange = file["/science/LSAR/SLC/swaths/" + self.frequency + "/slantRange"][
            0
        ]
        self.frame.setStartingRange(slantRange)

        referenceUTC = (
            file["/science/LSAR/SLC/swaths/zeroDopplerTime"]
            .attrs["units"]
            .decode("utf-8")
        )
        ...
        self.frame.setNumberOfLines(
            file[
                "/science/LSAR/SLC/swaths/" + self.frequency + "/" + self.polarization
            ].shape[0]
        )
    ...
    def extractDoppler(self):
        ...
        quadratic = {}
        quadratic["a"] = (
            imgDop[int(imgDop.shape[0] / 2)]
            / self.frame.getInstrument().getPulseRepetitionFrequency()
        )
        quadratic["b"] = 0.0
        quadratic["c"] = 0.0
