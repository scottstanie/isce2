...
class UAVSAR_Polsar(Sensor):
    ...
    def _populateFrame(self, **kwargs):
        ...
        dtMid = DTU.timeDeltaToSeconds(tStop - tStart) / 2.0
    ...
    def _populateOrbit(self, **kwargs):
        ...
        for i in range(
            -5 * numgroup, int(nlines / numgroup) * numgroup + 5 * numgroup, numgroup
        ):
    ...
    def extractDoppler(self):
        ...
        d = numpy.matrix(
            [self.dopplerVals["Near"], self.dopplerVals["Mid"], self.dopplerVals["Far"]]
        ).transpose()
    ...
    def platformHeight(self):
        ...
        h = (
            self.metadata["Global Average Altitude"]
    ...
    def squintAngle(self):
        ...
        if h > startingRange:
            raise ValueError("Spacecraft Height too large (%s>%s)" % (h, startingRange))
        ...
        self.squint = math.degrees(math.atan2(sinSquint, math.sqrt(1 - sinSquint ** 2)))
    ...
    def peg(self):
        if not self._peg:
            ...
            self.logger.info(
                "UAVSAR_Polsar: peg radius of curvature = {}".format(self.elp.pegRadCur)
            )
            self.logger.info("UAVSAR_Polsar: terrain height = {}".format(th))
            self.logger.info(
                "UAVSAR_Polsar: mocomp II applied = {}".format(
